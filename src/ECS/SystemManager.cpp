...
//        Fawxy::Log::ECS->Info("Initialize SystemManager!");
    ...
    SystemManager::~SystemManager() {
        for (auto it = this->m_SystemWorkOrder.rbegin(); it != this->m_SystemWorkOrder.rend(); ++it){
            (*it)->~ISystem();
            *it = nullptr;
        }
...
//        Fawxy::Log::ECS->Info("Release SystemManager!");
    }
        ...
        while (!INDICES.empty()) {
            ...
            while (!member.empty()) {
                ...
                for (int i = 0; i < INDICES.size(); ++i) {
                    if (INDICES[i] != -1 && (bool(this->m_SystemDependencyMatrix[i][index])|| bool(this->m_SystemDependencyMatrix[index][i]))) {
                        member.push_back(i);
                        INDICES[i] = -1;
                    }
                }
...
//        Fawxy::Log::ECS->Info("Updating system work order");
...
//                    Fawxy::Log::ECS->Info("\t" + std::string(sys->GetSystemTypeName()));
                }
            }
        }
    }
